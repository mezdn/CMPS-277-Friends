...
using Friends.Models.Relations;
...
namespace Friends.Storage
{
    public class PostStore
    {
        ...
        public async Task<List<Post>> GetPosts(string username)
        {
            ...
            cmd.CommandText = @"SELECT id, username, content, timeOfCreation, groupId, categoryName FROM post";
            ...
            // TODO: foreach post get its group name
            return ret;
        }
        ...
        {
            var cmd = MySqlDatabase.Connection.CreateCommand() as MySqlCommand;
            cmd.CommandText = @"SELECT id, username, content, timeOfCreation, groupId, categoryName
        ...
        public async Task<PostLikedByUser> GetPostAndLike(int id, string username)
        {
            var cmd = MySqlDatabase.Connection.CreateCommand() as MySqlCommand;
            cmd.CommandText = @"SELECT id, username, content, timeOfCreation, groupId, categoryName, ( SELECT COUNT(*) FROM likes WHERE postId = @id AND username = @username )
                                FROM post WHERE id = @id";
            cmd.Parameters.AddWithValue("@id", id);
            ...
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                while (await reader.ReadAsync())
                {
                    ...
                    var postObject = new PostLikedByUser
                    {
                        Post = post,
                        LikedByUser = false
                    };

                    if (reader.GetFieldValue<long>(6) != 0)
                    {
                        postObject.LikedByUser = true;
                    }  
                    return postObject;
                }
            }
        ...
        public async Task UpdatePost(Post post, string username)
        {
            ...
            cmd.CommandText = @"UPDATE post SET content = @content, categoryName = @categoryName
                                WHERE id = @id AND username = @username;";
            cmd.Parameters.AddWithValue("@id", post.ID);
            cmd.Parameters.AddWithValue("@username", username);
            cmd.Parameters.AddWithValue("@content", post.Content);
